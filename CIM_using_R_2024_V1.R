...
mean(z)-mean(y)
boxplot(z,y)
t.test(z,y,alternative = "greater",var.equal = TRUE)
t.obs <- t.test(z,y,alternative = "greater",var.equal = TRUE)$statistic
...
for(b in 1:B)
{
x.boot <- sample(x,size = n + m,replace = FALSE)
z.boot <- x.boot[1:n]
y.boot <- x.boot[(n + 1):(n + m)]
t.boot[b] <- t.test(z.boot,y.boot,alternative = "greater",var.equal = TRUE)$statistic
}
